                                                ...
                                                <table id="table_Category" class="w-100 display nowrap row-border table table-bordered align-middle table-hover mb-0">
                                                    ...
                                                    <tr id="row{{$c->category_id}}">
                                                        <td class="text-center">
                                                            <input type="checkbox" name="chk[]" value="{{$c->category_id}}" id="{{$c->category_id}}" form="multiForm" class="form-check-input checkbox">
                                                        </td>
                                                        <th scope="row" class="text-center">{{++$keys}}</th></th>
                                                        <td class="text-start text-nowrap " id="categoryName{{$c->category_id}}">{{$c->category_name}}</td></td>
                                                        <td >
                                                            <div class="d-flex justify-content-center align-items-center">
                                                                <div class="mx-1">
{{--                                                                    onclick="edit('category/',{{$c->category_id}})"--}}
                                                                        <button  class="btn  edit-icon px-2 py-1" data-bs-toggle="modal" data-bs-target="#editModal{{$c->category_id}}" >
                                                                ...
                                                                <div class="mx-1 text-nowrap">
                                                                    {{-- <form action="{{route('category.destroy',$c->category_id)}}" method="POST">
                                                                        @csrf
                                                                        @method('DELETE')
                                                                        <button type="button" onclick="deleteConfirm({{$c->category_id}})" class="btn trash-icon px-2 py-1">
                                                                            <i class="bi bi-trash text-danger fa-fw"></i>
                                                                        </button>
                                                                    </form> --}}
                                                                    <button  class="btn  edit-icon px-2 py-1" data-bs-toggle="modal" onclick="delConfirm({{$c->category_id}})">
                                                                        <i class="bi bi-trash text-danger fa-fw"></i>
                                                                    </button>
                                                                </div>
                                                            </div>
...
@foreach($categories as $c)
    <div class="modal fade " id="editModal{{$c->category_id}}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog  modal-dialog-centered">
            <div class="modal-content text-primary">
                ...
                <div class="modal-body  border-0">
                  <form action="{{route('category.update',$c->category_id)}}" id="categoryForm{{$c->category_id}}" method="post">
                    @csrf
                    @method('put')
                    <input type="text" id="editId" name="category_id" value="{{$c->category_id}}" class="d-none" >
{{--                    <input type="text" id="editId" name="category_id" value="{{$c->category_id}}" class="d-none" >--}}
                ...
                <div class="modal-footer border-0">
                    ...
                    <button form="categoryForm{{$c->category_id}}"type="submit" class="btn btn-primary">Save</button>
                </div>
            </div>
        </div>
    </div>
