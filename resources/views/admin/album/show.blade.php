            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    <li class="breadcrumb-item">
                        <a href="{{ route('project.edit', $albums->project_id ) }}">
            ...
            <div class="pb-3 px-3">
                <div class="row create">
                    <div class="col-12 px-0">
                        ...
                        <div class="alert alert-success d-flex justify-content-between align-items-center animate__animated animate__fadeInDown my-2">
                            {{ Session::get('success') }}
                            <div class="text-end">
                        ...
                        <div class="row">
                            <div class="col-4 bg-white bg-opacity-10 vh-100">
                                ...
                                <div class="px-3 py-3">
                                    <form method="POST" action="{{ route('albumTest.update', ['projectId' => $albums->project_id, 'id' => $albums->id]) }}" enctype="multipart/form-data">
                                        @csrf
                                        @method('PATCH')
                                        ...
                                        <div class="form-group mb-3">
                                            ...
                                            <input type="text" name="albumName" class="create-input form-control @error('albumName') is-invalid @enderror" id="album" value="{{ old('album', $albums->title) }}" required>
                                            @error('albumName')
                                        ...
                                        <div class="form- mb-3">
                                            <label for="uploadFile" class="form-label text-white-50">Upload
                                                File:</label>
                                            <input id="files" type="file" name="uploadFile[]" class="create-file form-control " multiple>
                                            @error('uploadFile')
                                            <span class="invalid-feedback">{{ $message }}</span>
                                            @enderror
                                        </div>
                                        ...
                                        <button type="submit" class="btn btn-outline-primary btn-lg w-100 my-5">Save</button>
                                    </form>
                                </div>
                            </div>
                            <div class="col-8">
                                <div id="output" class="row row-cols-4 g-1 py-2">
                                    ...
                                    <div class="col img-div position-relative">
                                        ...
                                        <div class="bg-black bg-opacity-50 text-white w-100 position-absolute bottom-0 py-1 px-1 d-flex justify-content-between align-items-center">
                                            <span class="text-truncate pe-2">
                                                {{ $album->imageName }}
                                            </span>

                                            <a href="{{ asset('storage/images/album/' . $album->image) }}" download class="me-2" title="download">
                                                <i class="bi bi-download fw-bold"></i>
                                            </a>
                                        </div>
                                        <div class="position-absolute top-0 me-1 pointer delImg w-100 d-flex justify-content-end align-items-center">
                                            <form action="{{ route('albumImage.delete', ['albumId' => $albums->id, 'imageName' => $album->image]) }}" method="post">
                                                @csrf
                                                @method('DELETE')
