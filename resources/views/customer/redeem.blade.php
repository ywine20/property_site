                    ...
                    <ul class="nav nav-tabs">
                        <li class="nav-item">
                            <a class="nav-link  text-secondary"
                                href="{{ route('profile', Auth::guard('user')->user()->id) }}">Authorize Asset</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link  text-secondary"
                                href="{{ route('profile.setting', Auth::guard('user')->user()->id) }}">Setting</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link active text-secondary"
                                href="{{ route('profile.redeem', Auth::guard('user')->user()->id) }}">Redeem</a>
                        </li>
                    </ul>
                </div>
                <!-- END SUB NAV -->
                                ...
                                <div class="d-flex flex-column flex-md-row justify-content-center align-items-baseline ">
                                    <div class="w-100" id="redeemInput">
                                        <input type="text" name="code" maxlength="50"
                                            class="form-control form-control-lg @if (session('InvalidCode')) is-invalid @endif"
                                            placeholder="S34DFGH5HJ77YHFG" require>
                                    ...
                                    <button type="submit"
                                        class="btn btn-secondary btn-lg rounded rounded-1 text-primary fw-bold ms-0 ms-md-3 mt-3 mt-md-0 redeemSubmitBtn">Submit</button>
                                </div>
                            </div>
                        </form>

                        @if (session('redeemSuccess'))
                            <div class="alert alert-success alert-dismissible show mt-3 mt-md-5 animate__animated animate__fadeInDown"
                                role="alert">
                                <div
                                    class="d-flex flex-column flex-md-row justify-content-start align-items-center text-center text-md-start">
